...
// integers [Li, Ri, Hi], where Li and Ri are the x coordinates of the left and right
// edge of the ith building, respectively, and Hi is its height.
...
public class Solution {
    public List<int[]> getSkyline(int[][] buildings) {
        ...
        for (int[] building : buildings) {
            Node newNodeLeft = new Node(true, building[0], building[2]);
            Node newNodeRight = new Node(false, building[1], building[2]);
        ...
        for (Node node : nodes) {
            if (node.left) {
                if (heights.isEmpty() || node.height > heights.peek()) {
                    int[] newInts = new int[2];
                    newInts[0] = node.x;
                    newInts[1] = node.height;
                    ret.add(newInts);
                }
                heights.add(node.height);
            } else {
                if (node.height == heights.peek()) {
                    heights.poll();
                    // after removing, we only add if height drops, if height doesn't drop
                    //  it's the same line
                    if (heights.isEmpty() || node.height > heights.peek()) {
                        int[] newInts = new int[2];
                        newInts[0] = node.x;
                        newInts[1] = heights.isEmpty() ? 0 : heights.peek();
                        ret.add(newInts);
                    }
                } else {
                    // this is cheating - actally not, removing an element from heap is O(log n)
    ...
    class Node implements Comparable<Node> {
        ...
        public Node(boolean left, int x, int height) {
            this.left = left;
            this.x = x;
            this.height = height;
        }
        ...
        public int compareTo(Node o) {
            ...
            // both start, higher one first
            if (left && o.left) {
                return o.height - height;
            }
            // both end, higher one later
