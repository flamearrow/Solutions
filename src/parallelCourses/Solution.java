...
public class Solution {
    public int minimumSemesters(int N, int[][] relations) {
        int semester = 0;
        Set<List<Integer>> relationsSet = new HashSet<>();
        for (int[] relation : relations) {
            List<Integer> newRelation = new ArrayList<>();
            newRelation.add(relation[0]);
            newRelation.add(relation[1]);
            relationsSet.add(newRelation);
        }

        while (!relationsSet.isEmpty()) {
            if (removeAllCoursesWithoutPrerequisites(relationsSet, N)) {
                semester++;
            } else {
                // find a loop
                return -1;
    ...
    boolean removeAllCoursesWithoutPrerequisites(Set<List<Integer>> relationsSet, int N) {
        Set<Integer> coursesWithoutPrereq = new HashSet<>();
        Set<Integer> coursesWithPrereq = new HashSet<>();
        for (List<Integer> relation : relationsSet) {
            int firstCourse = relation.get(0);
            int secondCourse = relation.get(1);
            coursesWithoutPrereq.add(firstCourse);
            coursesWithoutPrereq.add(secondCourse);
            coursesWithPrereq.add(secondCourse);
        }
        for (Integer course : coursesWithPrereq) {
            coursesWithoutPrereq.remove(course);
        }
        // all courses have prereq, has a loop
        if (coursesWithoutPrereq.size() == 0) {
            return false;
        }
        Set<List<Integer>> newRelationSet =
                relationsSet.stream()
                        .filter(relation -> !coursesWithoutPrereq.contains(relation.get(0)))
                        .collect(Collectors.toSet());
        relationsSet.clear();
        relationsSet.addAll(newRelationSet);
        return true;
    }
